...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for food in newFood.asList():
          distanceFood = min(distanceFood, util.manhattanDistance(newPos, food))
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getMinValue(self, gameState, depth, agent):
        ...
        for action in actions:
            ...
            resCosts.append(cost)

        return sum(resCosts) / float(len(resCosts)), None
        util.raiseNotDefined()
...
def betterEvaluationFunction(currentGameState):
   ...
   # costGhost = 0
    ...
    for ghost in newGhostStates:
      ...
      if distance == 0 and ghost.scaredTimer == 0:
        return float('-inf')
      if distance == 1 and ghost.scaredTimer == 0:
        distanceFood -= 1000
      if distance < ghost.scaredTimer and ghost.scaredTimer > 0:
        distanceFood += 50

    
    return currentGameState.getScore() + 1.0/(distanceFood)
